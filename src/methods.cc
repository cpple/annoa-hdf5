...
namespace NodeHDF5 {
  ...
  void Methods::getDatasetAttributes(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    herr_t      err = H5Oget_info_by_name(group->id, *dset_name, &object_info, H5O_INFO_NUM_ATTRS, H5P_DEFAULT);
    if (err < 0) {
      v8::Isolate::GetCurrent()->ThrowException(
  ...
  int Methods::getNumAttrs() {
    ...
    H5Oget_info(id, &oinfo, H5O_INFO_NUM_ATTRS);
  ...
  H5O_type_t Methods::childObjType(const char* objname) {
    ...
    herr_t ret_value = H5Oget_info_by_name(id, objname, &objinfo, H5O_INFO_NUM_ATTRS, H5P_DEFAULT);
